...
export default () => {
  ...
  return (
    <List
      ...
      boxShadow={"md"}
    >
      <Link to="/n" fontSize={"xl"} fontWeight={"bold"}> <Text fontSize={"xl"} fontWeight={"bold"} paddingBottom={"10px"}>
        공지사항
      </Text ></Link >
        ...
        noticepost.map((posts) => {
          return (<ListItem key={posts._id}>
            <Grid templateColumns="1fr 5fr 1fr" paddingBottom={"3px"} fontSize="13px">
              <Box>[공지]</Box>
              <Box _hover={{ textDecoration: "underline" }}><Link to={`/n/id=${posts._id}`}>{posts.title}</Link></Box>
