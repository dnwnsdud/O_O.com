import { Box, Button, Card, CardBody, CardHeader, Divider, Grid, Heading, Link, List, ListItem, Stack, Text } from "@chakra-ui/react";
    ...
    return <div>
        <Stack bg={'gray.50'} width={"45%"} margin={"auto"} spacing={8} h={"180vh"}>
            <Box display={"flex"} flexDirection={"column"} bg={'white'} h={"20%"} borderRadius={"10px"} boxShadow={"md"}>
                ...
                <Button border={"2px solid #0b0b0d"} onClick={() => { nav('/r') }}>이전결과</Button>
            </Box>
            ...
            <List bg={'white'} h={"12%"} borderRadius={"10px"} spacing={.5} paddingLeft={6} paddingRight={6} paddingTop={3} paddingBottom={3} boxShadow={"md"}>
                <Text fontSize={"xl"} fontWeight={"bold"}>실시간 인기글</Text>
                <Divider orientation='horizontal' />
                <ListItem><Link>Lorem ipsum dolor sit amet</Link></ListItem>
                <Divider orientation='horizontal' />
                <ListItem><Link>Consectetur adipiscing elit</Link></ListItem>
            ...
            <Grid flexWrap={"wrap"} gap={5} h={"20%"} templateColumns={"1fr 1fr"} justifyContent={"space-between"}>
                <Card size='md' cursor={"pointer"} _hover={{ bg: "black" }} onClick={() => {
                    nav("/b")
                }}>
                    <CardHeader>
            ...
            <Box cursor={"pointer"} bg={'white'} h={"20%"} borderRadius={"10px"} boxShadow={"md"} onClick={() => { nav("/st") }}
            >포인트</Box>
        </Stack>
    </div>
}