...
export default () => {
  ...
  return (
    ...
    >
      ...
      {bestpost.length === 0 ? (
        ...
        bestpost.map((posts) => (
          <ListItem key={posts._id}>
            <Grid
              key={`grid_${posts._id}`}
              templateColumns="1fr 2fr 1fr 0.5fr 0.5fr"
              paddingBottom={"3px"}
            >
              <Box
                key={`box_${posts._id}`}
              ...
              >
                ...
                >
                  <Link to={`/b/id=${posts._id}`}>{posts.title}</Link>
                </Box>
                <Box key={`comment_${posts._id}`} fontSize="10px" opacity={0.7}>
                  [{posts.comment.length}]
                </Box>
              </Flex>
              <Box
                key={`nickname_${posts._id}`}
                margin={"auto"}
                fontSize={"13px"}
              >
                {posts.nickname}
              </Box>
              <Box key={`like_${posts._id}`} margin={"auto"} fontSize={"13px"}>
                {posts.like}
              </Box>
              <Box key={`count_${posts._id}`} margin={"auto"} fontSize={"13px"}>
                {posts.count}
              </Box>
            </Grid>
            <Divider orientation="horizontal" />
      ...
      <Box opacity={0.5} fontSize={"10px"} marginLeft={"83%"}>
        인기글 = 추천수 * 2 + 조회수
      </Box>
    </List>
  );
