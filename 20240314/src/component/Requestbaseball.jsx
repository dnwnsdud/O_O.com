...
export default () => {
  ...
  const currentPosts = baseballData.slice(indexOfFirstPost, indexOfLastPost);
      ...
      fetch("/api/requestlist")
        ...
        .then((data) => {
          if (data) {
            ...
            setTotalPosts(data.baseballCount);
          } else {
  ...
  console.log(currentPosts, "배열로 잘 나오지롱~~~~");
  return (
    <Center>
      <Stack w="100%">
        <Box paddingTop="10px" fontWeight="bold" fontSize="1.2rem" textAlign="center">야구</Box>
        <List w="100%">
